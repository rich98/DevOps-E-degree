...
if ! mount -t ext4 /dev/sda1 /backup; then
    echo "Failed to mount backup folder"
    exit 1
fi
...
for dir in "${backup_dirs[@]}"; do
    if ! rsync -a --delete --exclude-from=<(printf '%s\n' "${exclude_dirs[@]}") --max-size=$max_file_size "$dir" "$backup_folder" --log-file="$log_file" 2>&1; then
...
if [ "$compression" = true ]; then
    if ! tar -czf "$backup_folder.tar.gz" -C "$backup_folder" .; then
        echo "Failed to compress backup folder"
        exit 1
    fi
    rm -rf "$backup_folder"
...
if ! $gpg_command "$backup_folder"; then
    echo "Failed to encrypt backup folder"
    exit 1
fi
...
if ! find "$backup_dir" -type f ! -name "$(basename "$backup_folder.gpg")" ! -name "$(basename "$log_file")" -delete; then
    echo "Failed to clean up backup directory"
    exit 1
fi
...
#Umount the folder Its not intended for this drive to be connected during working hours - protect from ransomware
# umount -l /dev/sda1
if ! umount -f /backup; then
    echo "Failed to unmount backup folder"
    exit 1
fi
